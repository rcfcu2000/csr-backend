...
func (i *RespProductCrowd10List) GetData(r ReqCrowdSaAllSearch) error {
	...
	sqlp.SetSql(SQL_CROWD_ProductCrowd10List)
	(&r).SetToSQLProccesor(sqlp)
	mainSql := sqlp.GetResult()

	fmt.Println(" SQL_CROWD_ProductCrowd10List sql\n", mainSql)
...
func (i *RespProductCrowd10List) Prodata() {

}

const SQL_CROWD_ProductCrowd10List = `  -- product_id, product_name, crowd_type, 
select   product_id, product_name ,   sum(paid_amount) as gmv, sum(visitors) as visitors_count,
...
group by  product_id, product_name  -- crowd_type --   
